...
const SideBar = memo( function SideBar() {
    ...
    const handleViewProject = useCallback(id => {dispatch(toDoSliceActions.viewProject(id));}, [dispatch]);
    const handleEditProject = useCallback(id => {dispatch(toDoSliceActions.editProject(id));}, [dispatch]);
    const handleRemoveProject = useCallback(id => {dispatch(toDoSliceActions.removeProject(id));}, [dispatch]);
        ...
        <div>
            <ul className="flex flex-col text-gray-500">
                {_.isArray(projects) && projects.map(project =>
                    ...
                    >
                        <div className='cursor-pointer' onClick={() => handleViewProject(project.id)}>
                            <strong className="truncate">{project.title}</strong>
                        </div>
                        <div className="flex flex-row">
                            <div className='cursor-pointer' onClick={() => handleEditProject(project.id)}>
                                <svg className="w-6 hover:w-7" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                                    <path
                            ...
                            <div className='cursor-pointer ml-2' onClick={() => handleRemoveProject(project.id)}>
