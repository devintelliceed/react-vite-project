...
const Input = forwardRef( function Input ({ label, isTextArea, name, ...attr }, ref ) {
    return <p className='className="flex flex-col gap-1 my-4'>
        <label className={labelClasses} htmlFor={name+label}>{label}</label>
        {isTextArea ? <textarea ref={ref} className={inputClasses} id={name+label} name={name} {...attr} />
            : <input ref={ref} id={name+label} name={name} className={inputClasses} {...attr} />}
    </p>
});
Input.propTypes = {
